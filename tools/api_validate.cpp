...
[[nodiscard]] float worstOrientationWeightSum() {
    const auto kernel = engine::debug::buildGaborKernel();
    float worst = 0.0F;
    for (int o = 0; o < engine::debug::kGaborOrientations; ++o) {
        float sum = 0.0F;
        for (int tap = 0; tap < engine::debug::kGaborTaps; ++tap) {
            sum += kernel[(static_cast<std::size_t>(o) * engine::debug::kGaborTaps) + static_cast<std::size_t>(tap)];
        }
...
ENGINE_CHECK(aov_tables_are_total_and_consistent, Fast, Exact) {
    ctx.plan(kAovCount * 3);
    for (int i = 0; i < kAovCount; ++i) {
        const auto aov = static_cast<AovId>(i);
        const int channels = engine::debug::aovChannels(aov);
        ENGINE_EXPECT(ctx, channels >= 1 && channels <= 3,
                      std::string(engine::debug::kAovNames[i]) + " channels=" + std::to_string(channels));

        const engine::debug::PathTracedLane traced = engine::debug::pathTracedLane(aov);
        const engine::debug::GBufferLane raster = engine::debug::gbufferLane(aov);
        const AovSource source = engine::debug::aovSource(aov);
        ...
        ENGINE_EXPECT(ctx, ownedCorrectly, std::string(engine::debug::kAovNames[i]) + " lane/source disagree");

        // aovNeedsLightTransport is derived from aovSource; this pins the derivation itself.
        ENGINE_EXPECT(ctx, engine::debug::aovNeedsLightTransport(aov) == (source != AovSource::GBuffer),
                      std::string(engine::debug::kAovNames[i]) + " transport flag disagrees with source");
    }
}
...
ENGINE_CHECK(aov_names_round_trip, Fast, Exact) {
    ctx.plan(kAovCount + 4);
    for (int i = 0; i < kAovCount; ++i) {
        ENGINE_EXPECT(ctx, engine::debug::aovIdFromName(engine::debug::kAovNames[i]) == static_cast<AovId>(i),
                      std::string("name does not resolve: ") + engine::debug::kAovNames[i]);
    }
    ENGINE_EXPECT(ctx, engine::debug::aovIdFromName("bounce-count") == AovId::BounceCount, "hyphen form");
    ENGINE_EXPECT(ctx, engine::debug::aovIdFromName("BOUNCE_COUNT") == AovId::BounceCount, "upper snake form");
    ENGINE_EXPECT(ctx, engine::debug::aovIdFromName("indirectspecular") == AovId::IndirectSpecular, "run-together form");
    ENGINE_EXPECT(ctx, engine::debug::aovIdFromName("not an aov") == AovId::Count, "unknown name must not resolve");
}
...
ENGINE_CHECK(gabor_bank_rejects_dc, Fast, Exact) {
    ctx.plan(1);
    ...
    ENGINE_EXPECT(ctx, worst < 1e-6F, "worst orientation weight sum " + std::to_string(worst));
}
...
ENGINE_CHECK(filters_are_zero_on_a_constant_field, Fast, Exact) {
    ctx.plan(2);
    engine::scene::ThreadPool pool(static_cast<unsigned int>(ctx.threads()));
    ...
    const HdrImage sobel = engine::debug::sobelAov(flat, pool);
    ...
    ENGINE_EXPECT(ctx, worstSobel == 0.0F, "peak Sobel response " + std::to_string(worstSobel));

    const HdrImage gabor = engine::debug::gaborAov(flat, pool);
    ...
    ENGINE_EXPECT(ctx, worstGabor < 1e-6F, "peak Gabor response " + std::to_string(worstGabor));
}
...
ENGINE_CHECK(sobel_step_edge_matches_closed_form, Fast, Exact) {
    ctx.plan(4);
    engine::scene::ThreadPool pool(static_cast<unsigned int>(ctx.threads()));
    ...
    const HdrImage sobel = engine::debug::sobelAov(step, pool);
    const int midRow = kHeight / 2;
    ENGINE_EXPECT(ctx, texelR(sobel, kEdge - 1, midRow) == 4.0F,
                  "left of edge " + std::to_string(texelR(sobel, kEdge - 1, midRow)));
    ENGINE_EXPECT(ctx, texelR(sobel, kEdge, midRow) == 4.0F,
                  "right of edge " + std::to_string(texelR(sobel, kEdge, midRow)));
    ENGINE_EXPECT(ctx, texelR(sobel, kEdge - 3, midRow) == 0.0F,
...
ENGINE_CHECK(luminance_returns_rec709_weights, Fast, Exact) {
    ctx.plan(4);
    engine::scene::ThreadPool pool(static_cast<unsigned int>(ctx.threads()));
    ...
    const HdrImage luminance = engine::debug::luminanceAov(primaries, pool);
    ENGINE_EXPECT(ctx, texelR(luminance, 0, 0) == engine::debug::kRec709LuminanceWeights.r, "red weight");
    ENGINE_EXPECT(ctx, texelR(luminance, 1, 0) == engine::debug::kRec709LuminanceWeights.g, "green weight");
    ENGINE_EXPECT(ctx, texelR(luminance, 2, 0) == engine::debug::kRec709LuminanceWeights.b, "blue weight");
    // The three weights are defined to sum to 1, so white maps to 1 and the AOV is a true relative luminance.
    ENGINE_EXPECT(ctx, std::fabs(texelR(luminance, 3, 0) - 1.0F) < 1e-6F,
...
ENGINE_CHECK(hsv_inverts_to_rgb, Fast, Exact) {
    engine::scene::ThreadPool pool(static_cast<unsigned int>(ctx.threads()));
    ...
    const HdrImage hsv = engine::debug::hsvAov(image, pool);
    ...
    for (std::size_t i = 0; i < samples.size(); ++i) {
        ...
        ENGINE_EXPECT(ctx, worst <= bound, "round-trip error " + std::to_string(worst));
    }
}
...
ENGINE_CHECK(every_aov_renders_finite, Slow, Exact) {
    ctx.plan(kAovCount);
    std::string error;
    const auto renderer = engine::api::HeadlessRenderer::open(ASSET_ROOT_DIR, "scenes/cornell.json", error);
    if (!renderer) {
        ENGINE_EXPECT(ctx, false, "scene load failed: " + error);
        return;
    }
    ...
    for (int i = 0; i < kAovCount; ++i) {
        ...
        std::vector<float> buffer(static_cast<std::size_t>(kWidth) * kHeight *
                                  static_cast<std::size_t>(engine::debug::aovChannels(aov)));
        float* pointer = buffer.data();
        if (!renderer->render(request, std::span<float* const>(&pointer, 1), error)) {
            ENGINE_EXPECT(ctx, false, std::string(engine::debug::kAovNames[i]) + ": " + error);
            continue;
        }
        const bool finite = std::all_of(buffer.begin(), buffer.end(),
                                         [](float value) { return std::isfinite(value); });
        ENGINE_EXPECT(ctx, finite, std::string(engine::debug::kAovNames[i]) + " contains a non-finite value");
    }
}
...
ENGINE_CHECK(multi_aov_request_matches_single_aov_requests, Slow, Exact) {
    ...
    const auto makeRequest = [&](const std::vector<AovId>& aovs) {
        return engine::api::HeadlessRenderer::Request{
            .camera = renderer->defaultCamera(),
            .width = kWidth,
            .height = kHeight,
            .samples = 4,
    ...
    for (std::size_t i = 0; i < combined.size(); ++i) {
        ...
        if (!renderer->render(makeRequest({combined[i]}), std::span<float* const>(&pointer, 1), error)) {
            ENGINE_EXPECT(ctx, false, "single render failed: " + error);
            continue;
        }
        ENGINE_EXPECT(ctx, alone == together[i],
                      std::string(engine::debug::kAovNames[static_cast<int>(combined[i])]) +
                          " differs between a combined and a single-AOV request");
    }
}

ENGINE_CHECK_MAIN("api")
