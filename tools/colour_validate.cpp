...
ENGINE_CHECK(cie_tables_sit_on_their_normalisation_points, Fast, Exact) {
    ctx.plan(2);
    int peak = 0;
    ...
    ENGINE_EXPECT(ctx, cie::wavelengthNm(peak) == 555.0 && cie::tableRow(peak).colourMatching.y == 1.0, detail);
    ...
    ENGINE_EXPECT(ctx, d65At560 == 100.0, detail);
}
...
ENGINE_CHECK(rec709_matrix_reproduces_primaries_and_white, Fast, Exact) {
    ctx.plan(5);
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        ENGINE_EXPECT(ctx, error <= kDoubleRoundoff, detail);
    }
    cie::Spectrum flat;
    flat.fill(1.0);
    ...
    ENGINE_EXPECT(ctx, whiteError <= kDoubleRoundoff, detail);
    ...
    ENGINE_EXPECT(ctx, greyError <= kDoubleRoundoff, detail);
}

double rendererAverage(const engine::scene::BsdfParams& params, int channel) {
    return tools::reference::cosineAverageFresnel(
        [&](double mu) { return static_cast<double>(engine::scene::fresnelAtViewAngle(params, static_cast<float>(mu))[channel]); });
}
...
ENGINE_CHECK(chrome_matches_measured_chromium, Fast, Exact) {
    ctx.plan(12);
    const auto table = tools::metal_fit::loadNkTable(kChromiumTable);
    const auto material =
        engine::config::loadMaterialConfig((std::filesystem::path(ASSET_ROOT_DIR) / "materials" / "chrome.json").string());
    const auto fit = table ? tools::metal_fit::fitGulbrandsen(*table, Interpolation::Wavelength) : std::nullopt;
    if (!fit || !material) {
        for (int i = 0; i < 12; ++i) {
            ENGINE_EXPECT(ctx, false, "chromium table, fit or chrome.json unavailable");
        }
        return;
    }
    const engine::scene::BsdfParams params{.baseColor = material->diffuseColour,
    ...
    const glm::vec3 normal = engine::scene::fresnelAtViewAngle(params, 1.0F);
    char detail[200];
    for (int c = 0; c < 3; ++c) {
        ...
        ENGINE_EXPECT(ctx, material->diffuseColour[c] == static_cast<float>(fit->reflectivity[c]), detail);
        ...
        ENGINE_EXPECT(ctx, material->edgeTint[c] == static_cast<float>(fit->edgeTint[c]), detail);
        ...
        ENGINE_EXPECT(ctx, normalError <= kFloatResolution, detail);
        ...
        ENGINE_EXPECT(ctx, averageError <= kFloatResolution, detail);
    }
}
...
ENGINE_CHECK(nk_table_and_fit_reject_invalid_input, Fast, Exact) {
    ctx.plan(9);
    ENGINE_EXPECT(ctx, tools::metal_fit::loadNkTable(kChromiumTable).has_value(), "shipped chromium table rejected");
    ENGINE_EXPECT(ctx, loadText("engine_colour_crlf.csv", "0.30,1,1\r\n0.90,1,1\r\n").has_value(), "CRLF table rejected");
    ENGINE_EXPECT(ctx, !loadText("engine_colour_gap.csv", "0.30,1,1\n0.80,1,1\n"), "table ending at 800 nm accepted");
    ENGINE_EXPECT(ctx, !loadText("engine_colour_order.csv", "0.30,1,1\n0.50,1,1\n0.50,1,1\n0.90,1,1\n"),
                  "repeated wavelength accepted");
    ENGINE_EXPECT(ctx, !loadText("engine_colour_k.csv", "0.30,1,1\n0.90,1,-0.1\n"), "negative k accepted");
    ENGINE_EXPECT(ctx, !loadText("engine_colour_n.csv", "0.30,0,1\n0.90,1,1\n"), "zero n accepted");
    ENGINE_EXPECT(ctx, !loadText("engine_colour_row.csv", "0.30,1,1\n0.90 1 1\n"), "malformed row accepted");
    ...
    ENGINE_EXPECT(ctx, mirror && !tools::metal_fit::fitGulbrandsen(*mirror, Interpolation::Wavelength),
    ...
    ENGINE_EXPECT(ctx, step && !tools::metal_fit::fitGulbrandsen(*step, Interpolation::Wavelength),
                  "average outside the edgeTint range accepted");
}

}  // namespace

ENGINE_CHECK_MAIN("colour")
