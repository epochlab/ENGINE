// Standalone energy-conservation check for Phase 4's shading model (pbr.frag): a furnace test illuminates a surface uniformly from every direction with constant radiance L0 and asserts the outgoing radiance never exceeds L0 -- a surface must never reflect more energy than it received. Also checks the SH-9 diffuse IBL path against its one exact closed-form case (a uniform environment). No test framework exists in this repo -- follows test_pattern.cpp's convention of a plain CLI tool with a non-zero exit on failure.
//
// The GGX BRDF terms below are a hand-duplicated C++ re-implementation of pbr.frag's GLSL functions of the same name: GL4.1/C++20 have no shared-source mechanism without a codegen step, so this is the concrete backstop for the DFG polynomial fit and the multi-scatter compensation formula -- keep it in sync with pbr.frag by hand.
...
glm::vec3 multiScatterCompensation(const glm::vec3& f0, const glm::vec2& dfg) {
    ...
    // Must match pbr.frag's multiScatterCompensation exactly, including the safety ceiling -- see its comment for why it's needed.
    return glm::min(compensation, glm::vec3(1.1F));
}

// Monte Carlo furnace test: uniform hemisphere sampling (pdf=1/(2*pi)) of a unit-radiance (L0=1) incident field, integrating the full diffuse+specular BRDF response. Returns Lo(v), which must not exceed 1.0 for an energy-conserving BRDF.
float furnaceReflectance(float roughness, float metallic, float ndotV, std::mt19937& rng) {
    ...
    // NdotV-based (not per-sample VdotH-based) diffuse/specular energy split -- must match pbr.frag's evaluateDirectLighting exactly.
    ...
    for (int i = 0; i < kSampleCount; ++i) {
        // Uniform hemisphere sampling (PBRT-style inversion): z=u1, r=sqrt(1-u1^2), phi=2*pi*u2, pdf=1/(2*pi).
...
bool checkPunctualSweep() {
    std::mt19937 rng(42);
    // Sized to the analytic DFG's measured Ess underestimate at high roughness/near-1 F0 (see multiScatterCompensation's comment), not to Monte Carlo noise: uniform hemisphere sampling under-samples a sharp GGX lobe at low roughness (e.g. 0.05), so those rows read low rather than converge tightly -- harmless for this check (it only asserts an upper bound, and under-reporting can't produce a false failure), but not a precise reading either.
...
// Uniform-environment SH check: with constant environment radiance L0, irradiance must equal exactly pi*L0 (the hemispherical integral of a constant times cos(theta)), independent of surface normal -- the one exact closed-form case for projectIrradianceSH9/evaluateIrradianceSH9.
bool checkUniformEnvironmentSH() {
    ...
    for (const glm::vec3& n : testNormals) {
        ...
        // 2% relative tolerance for the grid quadrature's discretization error at this resolution.
