...
struct EnvironmentConfig {
    ...
    // opposed to just the camera-visible background -- see pathtracer::scene::LightSet. Optional, default
...
// A rectangular area light (Arnold quad_light semantics): origin is one corner, edge0/edge1 span the two sides, radiance = color * intensity is emitted from the face whose outward normal is normalize(cross(edge0, edge1)). See pathtracer::scene::QuadLight.
