...
// One renderPathTraced() call's raw output -- what a single pass computes, and what PathTraceDriver republishes in full on every accumulated pass. Single-channel fields are broadcast to RGB (alpha=1), matching HdrImage's fixed 4-floats/texel layout so every field can go straight through Texture::createFromFloatPixels unchanged. Every field here is a per-sample quantity averaged across the call's samples, and re-averaged across accumulated passes by the driver. The primary-hit G-buffer AOVs (depth/worldPos/normal/albedo/metallic/roughness/tangent/objectId/alpha/uv/geomNormal/IOR) are NOT here: rasterizer.h's RasterGBuffer is their only producer, refreshed synchronously on the render thread every trigger change, and main.cpp's selectPathTracedImage routes every one of them there -- the path-traced copies were computed, stored and published to no reader at all. Wireframe/BoundingBox are likewise rasterizer.h-only. Fresnel is the one that moved the other way, and for the opposite reason: its path-traced form is not a copy of the rasterizer's but a different estimator (see the field below), so there is something here the G-buffer cannot produce.
struct PathTraceResult {
    ...
    // Expected Fresnel reflectance at the primary hit over the VISIBLE microfacet normal distribution -- E[F(wo.wh)] for wh ~ D_vis(wo), one VNDF draw per sample (bsdf.h's fresnelAtMicrofacet), averaged and re-averaged exactly as the lanes above are. This is the Fresnel the microfacet BSDF evaluates (Walter et al. 2007; Heitz 2018), so unlike the macro-normal value the rasterizer used to write it is roughness-dependent: a rough surface reports the lobe-width-weighted mean rather than one point on the grazing ramp, and it collapses back onto that point as alpha reaches its kMinAlpha floor. Full RGB, not the (F, 1-F, 0) packing it replaces -- a conductor's Fresnel is chromatic by construction (Gulbrandsen 2014's edgeTint inverts to a vec3 complex IOR), and discarding two channels discarded exactly what the term is shown to debug.
    // 0 where there is no BSDF vertex at bounce 0: the camera ray missed, or hit an emitter quad, which returns before the vertex is shaded. The same background convention shadow carries, and unlike the rasterizer, which had no emitter concept and wrote a surface value there.
