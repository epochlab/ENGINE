...
// IEEE 754 binary16, the compiler's native half: AArch64 widens it with one FCVT (FCVTL for 4 lanes), unlike
// Imath::half's 256 KB lookup table, and its bit layout is OpenEXR's HALF.
// Imath::half's 256 KB lookup table, and its bit layout is identical.
using Half = _Float16;
static_assert(sizeof(Half) == 2, "binary16 must be 2 bytes to alias OpenEXR HALF data");

// binary16 format parameters (IEEE 754-2008 Table 3.2): precision p = 11, emax = 15. Derived here because libc++
// does not specialise numeric_limits<_Float16>.
...
// Storage precision of image data: Float16 (11-bit significand, unit roundoff 2^-11, finite max 65504) or Float32,
// an exact copy of the float source.
