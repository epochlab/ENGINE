...
// CPU-side decode of a linear scanline EXR, shared by Texture's GPU upload path (createFromExr) and any CPU-side consumer that needs the same pixel data without a GPU round-trip (e.g. SH irradiance projection over an environment map). Row 0 is the top of the image, matching EXR's own scanline order and glTF's v=0-at-top UV convention -- no row flip.
struct HdrImage {
    int width = 0;
    int height = 0;
