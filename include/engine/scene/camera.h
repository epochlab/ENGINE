...
//
// Convention: right-handed, +Y up, -Z forward in view space. At yaw=0, pitch=0 the camera looks down world -Z with +Y up and +X right — this matches GLM's own conventions (glm::lookAt, glm::perspective) and glTF's coordinate system (relevant once Phase 2's glTF loader exists).
//
...
public:
    ...
    // yawDegrees/pitchDegrees are authored in degrees (more ergonomic at call sites than radians); converted once here and stored as radians, since every consumer (the Euler-to-forward-vector trig) needs radians.
    // aperture is an f-number (e.g. 2.8 for f/2.8), shutterSeconds is the exposure time (e.g. 1/125), iso is sensor sensitivity — the standard photographic exposure triangle, feeding ev100()/exposure() below.
    ...
    // Linear scene-radiance multiplier derived from ev100() (the Filament/Frostbite EV100 calibration convention), ready to apply before Stage F's OCIO display transform.
