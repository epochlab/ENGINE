...
// Every entry starts as a copy of `base` (the scene-wide material file); an instance whose MeshInstance::name has a materialOverrides entry gets that file's 11 material fields substituted. Renderer-only fields (samplesPerPixel/maxBounces/RR/aoMaxDistance) are never overridden -- they are not material properties and no material file carries them.
