...
public:
    ...
    explicit EnvironmentMap(engine::gfx::ImageTexture<3> image);
...
private:
    engine::gfx::ImageTexture<3> image_;  // the CDFs below are built from these stored values, so sampling density matches the radiance returned at either ScalarType
