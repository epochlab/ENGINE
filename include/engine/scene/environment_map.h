...
public:
    ...
    explicit EnvironmentMap(engine::gfx::ImageTexture image);
...
private:
    engine::gfx::ImageTexture image_;  // the CDFs below are built from these stored values, so sampling density matches the radiance returned at either ScalarType
