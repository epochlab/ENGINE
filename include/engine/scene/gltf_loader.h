...
// Parses path via cgltf, resolving each material's textures (relative to path's directory) through Texture::createFromExr -- this project's glTF assets ship linear EXR maps, not glTF's usual PNG/JPEG. Returns nullopt and logs to stderr on any failure (bad file, missing accessor, missing texture, non-triangle primitive): fails clearly rather than substituting a placeholder for something this loader doesn't yet support.
