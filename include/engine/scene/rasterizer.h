...
// out: owned by the caller and reused across calls, like threadPool. Its 13 images are reallocated only when width/height change and are otherwise cleared per row inside the parallel loop -- the same bytes the old per-call makeImage zeroing touched, but written in parallel, in the row about to be overwritten, instead of as 13 sequential full-image memsets beforehand. At 2048x1152 that allocate-and-zero was 566 MB per call.
