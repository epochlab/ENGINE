...
// Matches pbr.frag's kMaxLights (its uLightType[16]/uLightPositionOrDir[16]/
// uLightColor[16]/uLightRange[16] array uniforms) -- the two can't share a
// literal across the GLSL/C++ boundary, so both sides name it instead of
// repeating the bare number.
inline constexpr int kMaxLights = 16;

// One punctual light (Phase 4: direct analytic evaluation, no visibility
// rays). Directional lights are unit-length, infinite-distance sources
// (directionOrPosition points toward the light, no falloff); point
// lights are finite-position sources with windowed inverse-square
// falloff out to `range` metres (Karis 2013, "Real Shading in Unreal
// Engine 4" -- avoids both the 1/d^2 singularity at d->0 and an abrupt
// cutoff at the range boundary).
struct Light {
...
// (that's ProfileConfig): the model to load, the punctual light list,
