...
1. **Headless `pathtracer_core` library**: one static library, no GL/GLFW/imgui, with every executable linking it. Today the sources have no home but `add_executable(pathtracer ...)` and each of the 15 tool targets re-lists the subset it needs; `render_beauty`, the headless tool, still links glfw and all of imgui because `vendored_no_tidy` bundles imgui with `cgltf_impl.cpp`. There is nothing for a foreign runtime to bind to until this exists. Pure refactor — gated on a bit-identical `render_beauty` CRC.
2. **Retina sensor and synchronous step**: `agent.md` §2's point field — N rays from a position and heading, one sample each, no jitter, packing the active tier's colour channels plus depth and normal per point. Not the progressive accumulator: one synchronous call returning one frame, seeded per step so episodes replay exactly (the `--assert-deterministic` machinery already exists). `agent.md` §3's tiers 1–3 are G-buffer reads needing no path tracing, so the cheap tiers run on today's pathtracer; 4–5 are the existing integrator at 1 spp. Sized for a bounded-FOV grid (32²–128²), not a display frame.
3. **Python binding**: a module over `pathtracer_core` exposing scene load, tier selection and `step(position, heading, seed) -> ndarray`. The numpy buffer protocol makes `torch.from_numpy` zero-copy on the host, leaving one explicit `.to(device)` — at retina resolution that copy is ~100 KB and the render is the only cost. Must release the GIL around the step, and share one BVH and one thread pool across N vectorized environments, since a per-environment pool sized to `hardware_concurrency()` oversubscribes by N. Needs a decision first: pybind11, which is a new third-party dependency, or a plain C ABI driven by `ctypes`, which adds no dependency but hand-rolls the buffer protocol.
...
- **Display-texture upload rebuilds the whole image every published pass** (measured via the benchmark log, [components](components.md#benchmark-tooling)). `ensurePathTraceDisplayTexture` (`main.cpp`) re-uploads the full RGBA32F buffer whenever the cache key changes, firing once per completed pass. That is 37.7 MB at 2048x1152, measured at **3.6-7.4 ms, median 4.6 ms** on the render thread over the 128 uploads of one `pathtracer -bench` convergence. Most of it is the driver's float-to-half conversion, not the transfer (46% on cornell, 68% on the stump). `displayBitDepth: 32` measured `upload_ms` **0.54x [0.50, 0.83]** of 16 on cornell and **0.32x [0.31, 0.33]** on the stump (`results/wave7`), so a 16-bit display copy only pays off if the conversion leaves the render thread. Remaining options, none tried: convert to half on the trace workers and upload `GL_HALF_FLOAT`, a PBO so the copy leaves the render thread, or uploading only the tiles the pass actually rewrote. Decide each with `bench_compare run --metric upload_ms` over `pathtracer -bench`.
