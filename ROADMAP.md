...
Offline export is not one of these items and ships with them: `render_beauty --out-exr` writes any of the 27 AOVs to disk, which covers dataset generation and debugging.
...
- **Contact sheet export**: every AOV at once, as a single displayed sheet. The headless half of this now ships -- `HeadlessRenderer` runs both producers plus the filters in one call, so `render()` already returns any subset of the 27 together -- leaving only the viewer side, where `aovNeedsLightTransport` still parks one producer while the other is shown.
