...
The consumer is `notes/agent.md`'s retina sensor, feeding `notes/neural.md`'s cognitive loop. Transport #8 names the same consumer at the far end of the transport spine — a richer tier of this interface, not a separate one. Offline export is *not* this item and already ships: `render_beauty --out-exr` writes any of 27 AOVs to disk, which covers dataset generation and debugging.

1. **Headless `engine_core` library**: one static library, no GL/GLFW/imgui, with every executable linking it. Today the sources have no home but `add_executable(engine ...)` and each of the 15 tool targets re-lists the subset it needs; `render_beauty`, the headless tool, still links glfw and all of imgui because `vendored_no_tidy` bundles imgui with `cgltf_impl.cpp`. There is nothing for a foreign runtime to bind to until this exists. Pure refactor — gated on a bit-identical `render_beauty` CRC.
2. **Retina sensor and synchronous step**: `agent.md` §2's point field — N rays from a position and heading, one sample each, no jitter, packing the active tier's colour channels plus depth and normal per point. Not the progressive accumulator: one synchronous call returning one frame, seeded per step so episodes replay exactly (the `--assert-deterministic` machinery already exists). `agent.md` §3's tiers 1–3 are G-buffer reads needing no path tracing, so the cheap tiers run on today's engine; 4–5 are the existing integrator at 1 spp. Sized for a bounded-FOV grid (32²–128²), not a display frame.
3. **Python binding**: a module over `engine_core` exposing scene load, tier selection and `step(position, heading, seed) -> ndarray`. The numpy buffer protocol makes `torch.from_numpy` zero-copy on the host, leaving one explicit `.to(device)` — at retina resolution that copy is ~100 KB and the render is the only cost. Must release the GIL around the step, and share one BVH and one thread pool across N vectorized environments, since a per-environment pool sized to `hardware_concurrency()` oversubscribes by N. Needs a decision first: pybind11, which is a new third-party dependency, or a plain C ABI driven by `ctypes`, which adds no dependency but hand-rolls the buffer protocol.
...
- **Display-texture staging has no validator**: nothing asserts the cache key rebuilds on a new pass and not on an AOV change, that `displayStaging`/`displayColormap` re-size across the render-scale switch, or that the BounceCount scratch carries nothing over. GL-bound, so the logic needs extracting first.
...
**Performance** — each item justified by the benchmark log ([components](components.md#benchmark-tooling)).
- **Display-texture upload**: the engine now produces the half texels and uploads `GL_HALF_FLOAT`, removing the driver's conversion and halving the bytes — `upload_ms` 0.549 (5.59 → 3.06 ms at 2048x1152). Unresolved, and why this stays open: `pass_ms` read 1.031 in the same comparison over two rounds, which if it holds trades render-thread slack for trace-thread time and is a net loss. Suspected cause is the staging round trip the driver's in-place conversion did not have. The fix would be converting on the driver thread during accumulation, which needs the driver to know the displayed AOV — not a PBO, which can only defer the smaller transfer half. Tile-dirty upload is structurally impossible: a pass is a running mean, so every texel changes.
...
- **Refactor the over-length functions**: `readability-function-size` fires on `tracePath`, `renderPathTraced`, `sampleBsdf`, `computeLobeProbabilities`, `driverLoop`, `buildSphericalRectangle`, `loadProfileConfig` and four in `main.cpp`. These are hot-path functions and splitting them risks the image, so this needs its own bit-identical before/after verification. Skip `tracePath`/`renderPathTraced` if Large #3 is near. Sweep the two trivial idiom fixes in with it (`rotateAboutY` → `glm::rotate`, `ShadingFrame::toLocal`/`toWorld` → `glm::mat3`).
- **Documentation pass**: a user/build guide, an architecture reference for the module layout, and write-ups of the techniques in use.
- **Memory efficiency pass**: scene-texture storage is done on both axes — data type (`textureBitDepth`) and channel count (`ImageTexture<N>`). What remains is the framebuffers, where every `HdrImage` AOV is still float RGBA whatever it encodes, and `gltf_loader.cpp`'s de-indexed mesh soup, which stores every position twice.
...
- **Fisheye lens**: equidistant/equisolid-angle/orthographic/stereographic projection families in `primaryRay` (Kannala & Brandt 2006, [references](references.md)).
- **Physical camera filters**: CPL/polarising filters, which need polarised transport (Chandrasekhar 1960, [references](references.md)).
...
- **Contact sheet export**: every AOV at once, which needs both producers live in one frame; `aovNeedsLightTransport` parks one while the other is shown.
...
- **Example images of engine technology (gallery)**: small demo scenes, each isolating one feature, rendered and embedded in this README.
