...
uniform int uChannelView;  // 0=off(HSV) 1=H 2=S 3=V, applied to the HSV output: isolating source RGB first would give H=0/S=0
uniform float uExposure;  // pow(2, relativeExposureEv()), applied before HSV so exposure affects this AOV, not Beauty alone
...
void main() {
    vec3 beauty = texture(uHdrColor, vUv).rgb * uExposure;
