...
// Punctual lights (Phase 4: direct analytic evaluation, no visibility
// rays). Type 0=Directional (uLightPositionOrDir is the unit direction
// toward the light, uLightRange unused), Type 1=Point (position, range
// in metres for the windowed inverse-square falloff, Karis 2013).
// Matches engine::config::kMaxLights (scene_config.h) -- GLSL and C++
// can't share this literal directly, so both sides name it.
...
// Prefiltered IBL (Karis 2013 split-sum). Diffuse is SH-9 irradiance
// (Ramamoorthi & Hanrahan 2001) -- no texture, just 9 coefficients.
// Specular is a GGX-prefiltered cubemap mip chain (env_prefilter_pass.h)
// sampled at an explicit roughness-derived LOD, combined with the
// analytic split-sum DFG approximation (Lazarov 2013) -- no baked LUT.
uniform vec3 uShIrradiance[9];
uniform samplerCube uPrefilteredSpecular;
uniform float uPrefilteredSpecularMaxLod;
// User-controlled Y-axis (world up) rotation of the environment, in
// radians. The SH coefficients and prefiltered cubemap are baked once
// at startup in the environment's original orientation; rather than
// re-baking either on every UI change, the query direction is rotated
// by the inverse angle before each lookup (see rotateAboutY) -- exactly
// equivalent to rotating the environment itself, and free per-fragment.
// sky.frag applies the same rotation to its equirect sample direction
// so the background, diffuse, and specular all stay in sync.
...
// 14=Tangent 15=ObjectID 16=AO 17=Fresnel 18=IBL -- order matches the
// README's §3 AOV reference table, and must match main.cpp's AOV combo
// list. Sobel/Gabor (5/6) only need this shader to output Luminance
// (same as 4) -- the actual edge filtering is a second pass, see
// edge_filter.frag.
uniform int uAov;
...
// Smith height-correlated visibility term (Heitz 2014), pre-divided by
// the 4*ndotV*ndotL denominator -- callers combine D*Vis*F directly.
...
// Analytic split-sum DFG approximation (Lazarov 2013, "Getting More
// Physically Based Reflectance" -- the same polynomial fit Karis's UE4
// notes cite for mobile), returning (scale, bias) in place of a baked
// 2D LUT texture: reflectance = f0*scale + bias.
...
// Multiplicative energy-compensation factor for a single-scatter
// microfacet specular value (Turquin 2019, "Practical multiple
// scattering compensation for microfacet models" -- the simpler,
// direct-lighting-friendly alternative to Fdez-Aguera 2019's IBL-only
// additive treatment). dfg.x+dfg.y approximates the white-Fresnel (f0=1)
// directional albedo of the single-scatter GGX lobe -- the same
// roughness/ndotV-dependent quantity split-sum IBL already needs, reused
// here with no extra texture or pass. Applied to both punctual and IBL
// specular so they lose energy consistently.
vec3 multiScatterCompensation(vec3 f0, vec2 dfg) {
    float ess = clamp(dfg.x + dfg.y, 1e-3, 1.0);
    vec3 compensation = 1.0 + (f0 * ((1.0 / ess) - 1.0));
    // Safety ceiling: envBRDFApprox's cheap polynomial fit measurably
    // underestimates Ess relative to the true single-scatter GGX
    // integral across much of the roughness range (worst near
    // roughness=1, where its NdotV dependence vanishes entirely -- its
    // scale term goes to exactly 0 there), so the compensation factor
    // this Ess drives can overshoot energy conservation -- caught by
    // tools/furnace_test.cpp. A real importance-sampled LUT wouldn't
    // have this gap; the analytic approximation does. 1.1 keeps the
    // worst measured case (a perfectly white, f0=1 conductor -- no real
    // material reaches this) within the furnace test's tolerance, at
    // the cost of leaving some darkening uncompensated for such
    // near-perfect reflectors specifically.
    return min(compensation, vec3(1.1));
}

// Rotates v by angleRadians about the world Y axis (right-handed).
// Sampling the *unrotated* environment at rotateAboutY(d, -angle) gives
// the same result as sampling the environment rotated by +angle at the
// original direction d -- see uEnvRotationRadians's comment. Identical
// to sky.frag's copy (GLSL/GLSL can't share source across programs any
// more than the GLSL/C++ pairs elsewhere in this shader can).
...
// Hanrahan 2001). Basis functions and cosine-lobe convolution constants
// must match sh_irradiance.cpp's projectIrradianceSH9/evaluateIrradianceSH9
// exactly -- coeffs already has the A_l convolution folded in, so this
// is a flat dot product, not a second convolution step.
...
// Punctual direct lighting: analytic Cook-Torrance GGX (D, Smith-
// correlated visibility, Schlick Fresnel), evaluated directly against
// each light -- no importance sampling/Monte Carlo, that's Phase 5's
// recursive-tracing concern. Shares its BRDF with the IBL specular term
// below (evaluateAmbient) so direct and ambient specular stay
// consistent, and shares dfg (already computed once by shadeBeauty) so
// the multi-scatter compensation factor isn't recomputed per light.
vec3 evaluateDirectLighting(vec3 n, vec3 v, vec3 worldPos, vec3 baseColor, vec3 f0,
    ...
    // Diffuse/specular energy split tied to the view angle (NdotV), not
    // each light's half-vector (VdotH): with a per-light VdotH Fresnel,
    // diffuse can stay high for light directions that happen to keep H
    // close to N even while V itself is grazing, so diffuse and the
    // independently-grazing-boosted specular term can together exceed
    // the energy received (caught by tools/furnace_test.cpp, which
    // shares this same formula). Tying the split to NdotV instead keeps
    // it consistent with the ambient term's dfg-based split below.
    ...
    for (int i = 0; i < uLightCount; ++i) {
        ...
        if (uLightType[i] == 0) {
            ...
            // Windowed inverse-square falloff (Karis 2013, "Real
            // Shading in Unreal Engine 4"): avoids both the 1/d^2
            // singularity at d->0 and an abrupt cutoff at uLightRange.
...
// Ambient/IBL term: SH-9 diffuse irradiance + prefiltered-specular-cubemap
// sample (roughness -> LOD) x analytic split-sum DFG x multi-scatter
// compensation. AO is applied only here -- it occludes the ambient term,
// not direct light, now that an ambient term actually exists to occlude
// (previously a blanket multiplier over everything, see git history).
vec3 evaluateAmbient(vec3 n, vec3 v, vec3 baseColor, vec3 f0, float roughness, float metallic,
                      vec2 dfg, float aoSample) {
    vec3 msComp = multiScatterCompensation(f0, dfg);
    // Single-scatter specular reflectance estimate, reused both as the
    // specular term's weight and (via 1-this) as the diffuse
    // attenuation -- same (1-F) energy split as evaluateDirectLighting,
    // using dfg in place of a per-sample Fresnel since IBL has no
    // single light direction to evaluate one against.
...
void main() {
    ...
    if (uAov == 1) {
        ...
        // Fresnel/reflectance: Schlick term at the actual view angle,
        // isolating grazing-angle behaviour from the rest of shading.
        vec3 v = normalize(uCameraPos - vWorldPos);
        vec3 f0 = mix(texture(uSpecular, vUv).rgb, baseColor, metallic);
        float ndotV = max(dot(shadingNormal, v), 1e-4);
        color = fresnelSchlick(ndotV, f0);
    } else if (uAov == 18) {
        // IBL/ambient only: SH diffuse + prefiltered specular, no direct
        // lights -- isolates the Phase 4 ambient term this AOV exists to
        // debug (AO's re-scoped role, SH/prefiltered-cubemap plausibility).
        // Diffuse albedo is deliberately dropped (baseColor -> white)
        // so this reads as "light arriving here", not "object colour
        // under ambient light" -- specular keeps the real F0 (a
        // physical reflectance property, not albedo), so a metal's
        // tinted reflection still shows.
        vec3 v = normalize(uCameraPos - vWorldPos);
        vec3 f0 = mix(texture(uSpecular, vUv).rgb, baseColor, metallic);
        float ndotV = max(dot(shadingNormal, v), 1e-4);
        vec2 dfg = envBRDFApprox(roughness, ndotV);
        color = evaluateAmbient(shadingNormal, v, vec3(1.0), f0, roughness, metallic, dfg,
                                 texture(uAo, vUv).r);
    } else {
        // Beauty (0).
