...
// Pinned, not "-latest": empirically verified (compiled + ran against the
// installed library) that this config's "Un-tone-mapped" view is a pure
// colorimetric pass (0->0, 1->1, zero LUT textures), no filmic rolloff.
// Pinning avoids a future brew upgrade silently resolving "-latest" to a
...
// Wraps OCIO's generated GLSL (uniform decls + helper functions + the
// named OCIODisplay*(vec4) function) in this project's client fragment
// program. Exposure is applied here, before the OCIO function is called —
// not through OCIO's own dynamic-property/uniform system — matching
// TASK.md's "exposure uniform pow(2,ev) before OCIO call".
...
// No OCIO involved at all: exposure applied, then output directly with no
// display encode — lets 'L' cycle to a genuine unencoded state for direct
// comparison against the two LUTs, rather than only ever toggling between
// two encoded curves.
...
// Builds one LUT's fragment source via OCIO's real Display/View API
// (getProcessor(scene, display, view, direction)) using the
// "Un-tone-mapped" view — a genuine View entry on this config, not a
// colorspace-pair bypass. Both displays resolve to pure Matrix+Gamma OETF
// math (verified against OCIO's own source + empirical testing): no LUT
// ops, no filmic tone-mapping.
//
// OCIO::Exception here means our own config/display/view/style names are
// wrong — an internal defect, not recoverable input — so it exits
// immediately like Window/HdrFramebuffer do for unrecoverable
// configuration errors, rather than threading optional through callers.
std::string buildOcioFragmentSource(const char* display, const char* functionName) {
    try {
        ...
        // Defensive, not merely documentary: this design assumes zero LUT
        // textures (verified empirically against the installed OCIO
        // 2.5.2). If a future OCIO/config change ever introduces one,
        // silently skipping texture upload would render garbage; fail
        // loudly instead so the assumption break is caught immediately.
