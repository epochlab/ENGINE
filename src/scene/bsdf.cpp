...
// Authoring a real metal: reflectivity and edgeTint come from measured (lambda, n, k) via tools/metal_fit, which CIE 1931-integrates the spectral Fresnel under D65 into linear Rec.709 and fits r to the normal-incidence and g to the cosine-weighted average reflectance, never from eyeballing a colour.
...
// where real metals such as chrome.json's measured chromium (r ~0.55) sit. bsdf_validate compares
