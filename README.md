...
- **The transmit-side escape table is deliberately under-converged** (white slab 2.7% over): `tools/albedo_table.cpp` bakes `r`/`t`/`ravg`/`tavg` at 16x16 stratified samples per cell, the count they had when cost still mattered, even though offline they are free.
  ...
  - That localises the rough-transmission over-energy to the transmit compensation itself rather than to MIS double-counting, which is what the failure message assumes. Not to its normalisation, though: both multiple-scattering shares now integrate to their deficit exactly (CHANGELOG, one normaliser), and the slab at roughness 1 read 1.02710 before that and 1.02713 after -- at roughness 1 the old baked average already matched the row integral to 1e-4 -- which leaves the escape values themselves. Reproduce with `./build/albedo_table --samples 128`; the fix belongs with the rough-transmission items below, and the count goes up once it lands.
