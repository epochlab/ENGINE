...
- **Benchmark log**: `raster_bench` (`tools/raster_bench.cpp:252`) and `render_beauty` (`tools/render_beauty.cpp:592`) each print one-shot timing to stdout only, nothing persists between runs. Append each run (git SHA, CLI args, scene, timing) to a log so regressions are visible across runs. Until then a path-tracer timing claim has to be made by interleaving two builds by hand, as the AO obscurance change was: run-to-run drift on this workload exceeds a few percent, which is larger than most single-change effects.
