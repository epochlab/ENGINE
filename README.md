...
Defaults to `assets/scenes/cornell.json` if `--scene` is omitted; pass `--scene assets/scenes/tree.json` for the tree scene.
...
| Volumetric absorption | Beer-Lambert extinction (`transmissionColor` over `transmissionDepth`, Arnold `standard_surface`/OpenPBR convention) applied to a single-level medium stack while a path is inside a `transmissionFactor>0` material; at `transmissionDepth 0` there is no medium and the same colour is a constant on-surface tint instead | Tinted/coloured glass, thick or thin, without participating-media in-scattering (§5 Large item 2) |
...
| `transmissionColor` / `transmissionDepth` | The **only** tint on transmitted light (Arnold `standard_surface`/OpenPBR convention), realized in one of two mutually exclusive regimes selected by the depth. At `transmissionDepth > 0` it is the interior medium's Beer-Lambert absorption coefficient, `sigmaA = -log(transmissionColor)/transmissionDepth`, applied while a path is inside a `transmissionFactor>0` material. At `transmissionDepth 0` there is no interior medium and `transmissionColor` is a constant on-surface tint applied once per interface crossing, so a closed solid reads its square. Optional, default `[1,1,1]`/`0.0` -- Arnold's and OpenPBR's own default, and a true no-op in either regime (§2 Volumetric absorption) |
...
- **Screen capture to PNG**: dump the composited, LUT-applied Beauty framebuffer (`glReadPixels`, precedent at `main.cpp:727-728`, the post-filter AOVs' probe read) before `app.hud.render()` (`main.cpp:1008`) so HUD, crosshair (`hud_overlay.cpp:266`), and the pixel probe panel (`hud_overlay.cpp:381`) are excluded. No PNG encoder in the tree yet (`third_party/imgui`'s is unrelated); needs one added.
...
- **Rough-transmission coverage at `transmissionDepth 0`**: `checkOnSurfaceTransmissionTint` (`tools/integrator_validate.cpp`) authors both its slab and its sphere at roughness 0.02, whose `alpha` of 4e-4 is below the smooth threshold, so every reading it takes comes from `sampleBsdf`'s delta branch. Measured, not assumed: tinting *only* that branch and reverting both continuous sites fails `bsdf_validate` 54 times and leaves `integrator_validate` **green**, so the rough transmission path's tint has no integrator-level coverage at all and rests entirely on `bsdf_validate`'s analytic rows. Adding a rough row may be a one-line change -- at `ior 1.0` Walter refraction is straight through for any microfacet normal, so the expected reading should still be exactly `transmissionColor^2` -- but that is the thing to measure first: whether the transmissive multiple-scattering lobe activates at `eta = 1` and moves the expectation off the square. Not Quick until that reads back.
- **An instrument that can see the coat half of `F_avg`**: `coatAlbedo` taking `dielectricFresnelAvg(ior)` rather than Karis' mean of Schlick is worth at most 1.9e-4 on any furnace reading, below every tolerance in `bsdf_validate`, so reverting that half alone leaves all five validators **green** while it is the half that actually moves the cornell picture (1882 of 1886 changed channels). `checkAverageFresnel` asserts `dielectricFresnelAvg` directly and so pins the function, but nothing pins the *call site*: a revert there is invisible. Needs either a coat-specific energy identity tight enough to resolve 2e-4, or a direct assertion on `coatAlbedo`'s output against an independently integrated reference, the way `checkAverageFresnel` does for the average itself.
- **CIE-integrated fit for the chromium reflectivity/edge-tint triples**: the measured chromium recorded at `conductorIorFromReflectivity` (`bsdf.cpp`) -- reflectivity `[0.552, 0.555, 0.558]`, `edgeTint` `[0.555, 0.558, 0.672]` from Johnson & Christy 1974 via Gulbrandsen eq 14/15 -- is sampled at three representative wavelengths rather than integrated against the CIE colour matching functions, which is what an RGB channel actually is. Its 0.0048 round-trip error against the source `(n, k)` and its sensitivity to the chosen wavelength triple are both documented there. Integrating properly needs the CIE curves and a chosen RGB primary set, and would give every future measured metal one principled path from tabulated `(n, k)` to an authored `(r, g)` pair instead of a hand-picked triple.
...
6. **Spectral upgrade**: per-wavelength transport and hero-wavelength sampling (Wilkie et al. 2014). Dispersion itself now ships in the RGB form (`MaterialConfig::abbe` → `bsdf.cpp`'s `cauchyIor`, one hero *channel* per path), so what remains is the genuinely spectral part. Two named limitations of the RGB form set the agenda: (a) `kRgbWavelengthsNm` is one fixed wavelength per channel, which OpenPBR's own reference implementation notes produces "discrete RGB bands" rather than continuous rainbows -- the cheap fix is a stochastically drawn lambda per path, needing only that constant lookup replaced by a draw from each channel's spectral sensitivity, no transport change; (b) radiance itself is still RGB, so a true spectral integrator additionally needs spectral upsampling of RGB textures and environment maps (Jakob & Hanika 2019). (a) is Quick, (b) is this item. Likely offline-only given sample-budget cost.
...
- Wilkie, A. et al. (2014). Hero wavelength spectral sampling. Computer Graphics Forum: the sample-budget-bounding scheme named in §5's spectral-upgrade roadmap item, not yet implemented. The shipped dispersion commits a path to one RGB *channel* at its first dispersive interface rather than to a wavelength at path start, which is the same one-sample idea at RGB resolution and strictly cheaper, since a path that never meets dispersive glass keeps all three channels.
...
- Adobe. OpenPBR BSDF reference implementation (`adobe/openpbr-bsdf`), `openpbr_constants.h`: `OpenPBR_BaseRgbWavelengths_nm` = 620/540/450 nm, the representative per-channel wavelengths `kRgbWavelengthsNm` takes, and the source of the "discrete RGB bands" limitation named in §5's spectral-upgrade item.
