...
### Albedo table accuracy (isolated, moderate)

- **`checkCoatFresnelAvg` is now an instrument for `src/scene/albedo_table.inc`, not for `F_avg`**: `dielectricFresnelAvg` became a 3-node quadrature rule over the same `fresnelDielectric` the coat reflects by -- the conductor's existing nodes and weights, carried across unrefitted -- so its own error at the swept iors is 3.3e-6 to 2.2e-5, two orders under what the check can resolve. What the check's 2e-4 tolerance now spends is the table and the inversion: measured worst 1.2e-4 at ior 3.0, 7.0e-5 at 1.5. A regeneration of the table that loses accuracy trips here first.
  - The open item is therefore table accuracy. The reflect side is 128x128 with a measured 3.0e-5 quadrature residual and ~3.2e-5 bilinear error away from the first `mu` bin; the first bin is 7.1e-3 and is the obvious next lever, as is the `sqrt(mu)` axis warp.
  - Worth resolving rather than leaving: `F_avg` reaches `coatAlbedo` through `multiScatterTint(F_avg, Eavg)*(1-E(mu))`, and that route predicts a recovered residual of ~1.2e-3 from the table's error. The realised figure is 1.2e-4, 10x better, because the recovery's actual conditioning (`dC/dF`, now a printed column) is 0.81-0.91 rather than the 0.054 of the multiple-scattering route alone -- `coatAlbedoAvg`'s rescale carries most of the signal. The prediction was pessimistic by the ratio of the two routes.
  - Known limitation, bounded and documented rather than closed: three fixed nodes cannot resolve the `ior -> 1` boundary layer, where `F(0) = 1` for every `ior > 1` but collapses over a width `~sqrt(ior-1)`. Worst 5.9e-4 at `ior` 1.0057 against a truth of 1.8e-3; `checkAverageFresnel` asserts it as a separate named band so the working band stays at 1e-4. No shipped material is in it (`clay.json` 1.55, `glass.json` 1.5168).
