...
### Albedo table accuracy (closed)

Closed. `src/scene/albedo_table.inc`'s reflect side is 256x256, its `mu` axis uniform in `sqrt(mu)` as the escape tables' already was, and its interpolation error is now a measured, asserted quantity rather than a remembered one.

- **The instrument came first, and it corrected the premise.** `checkAlbedoTableInterpolation` (`tools/bsdf_validate.cpp`) reports four worsts -- each axis with the other held on exact nodes, the first `mu` cell split from the rest of its axis, and `Eavg`'s own 1-D lerp -- plus a control row on exact nodes where no interpolation happens, so the instrument's own error is printed rather than assumed. That control immediately read 5.2e-3, and a convergence probe showed the fault was the REFERENCE: `referenceDirectionalAlbedo`'s `psi` substitution compresses the horizon layer to width `~alpha*mu`, so at `mu = 1/127` it is ~1e-3 wide against a 96-panel width of 1.6e-2, every panel of the error sat in the last panel pair, and 3072 panels still left 3.6e-5. Gauss-Legendre's outermost node is `O(1/n^2)` from the endpoint, inside the layer: 1.5e-7 at 192 nodes. The recorded "7.1e-3 first bin / 3.2e-5 elsewhere" was wrong twice over -- the first cell was really 4.2e-2, and the 3.2e-5 was the ROUGHNESS axis, not `mu`.
- **Measured, before -> after**: first `mu` cell 4.2e-2 -> 1.8e-3; `mu` axis cells 1..n 6.9e-3 -> 2.2e-3; roughness axis 1.1e-3 -> 5.9e-4; `Eavg` lerp 1.9e-5 -> 4.6e-6. The control holds at 3.0e-5, which is the stored values' own quadrature residual and agrees with what the generator independently prints -- two rules, two files, one number. `checkCoatFresnelAvg`'s worst row went 1.2e-4 -> 5.1e-5 on the reference fix alone and -> 3.5e-5 on the table, tolerance 2e-4 -> 6e-5.
- **A warp moves cells, it does not add them**, which is why the resolution doubled with it. At 256x128 the `sqrt(mu)` axis made cells at `mu = 0.4` 1.27x wider than a uniform axis' and turned `checkCoatFresnelAvg` from 5.1e-5 into 8.7e-5 -- a regression on the instrument the table is read by, whose worst rows all sit at `mu` 0.4. 256 `mu` nodes buy that band back at 0.63x the original width and quarter the grazing layer at the same time. An alpha-dependent warp (Smith `G1` as the axis coordinate) was measured and rejected: at `alpha` 0.25 it compresses `mu` in [0.3, 1] into 12% of the axis, for 5.3e-4 against a uniform axis' 1.2e-5 where `E` is not flat.
- **`mu = 0` is a real node now**, not a nudge off it: `E(0, alpha) = 1` exactly for every `alpha`, the bake asserts it on all 256 rows, and the column that used to be the table's worst is its sharpest. That required `reflectAlbedo` to take the radical `G2/cosO` form -- mandatory, not stylistic, because `smithLambda`'s `max(cos^2, 1e-8)` clamp bites once node 1 is `mu = 6.2e-5`, and because the removable `1/mu` it divides out is what made the old form singular there.
- **What remains, bounded and documented rather than closed**: the three directional worsts all sit on the lowest-alpha rows (roughness <= 0.022) at `mu <= 4.4e-3`, where the lobe is a near-mirror and the layer is narrower than a cell however the axis is warped. That is sub-degree grazing on a mirror, cos-weighted to ~1e-8. Pushing further means more `mu` nodes for that region alone, and the quadrature residual (3.0e-5) is the floor everywhere else regardless.
- Cost: `.inc` 9.07 MB -> 11.0 MB, bake 4m25s. Render `cornell.json` 96 passes: linear RMSE 2.20e-4, relMSE 1.67e-7. Interleaved A/B, 24 rounds: `pass_ms` B/A **0.9998 [0.9934, 1.0041], not resolved** -- the added `sqrtf` and the 4x table are both under the noise. A 12-round run of the same pair reported "B slower by 2.04%" on a contended machine; `nivcsw` was 2.7x higher in it, which is what the wider interval was saying.
