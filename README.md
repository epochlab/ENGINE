...
| `chrome.json` | 1.0 | 0.0 | 0.05 / 0.045 | Polished **chromium**, not an idealised mirror: `diffuseColour` (which at `metallic=1` *is* `f0`, Gulbrandsen's reflectivity `r`) and `edgeTint` are both derived from Johnson & Christy 1974 measured n,k (refractiveindex.info `main/Cr/nk/Johnson.yml`) through the paper's own eq 14/15, at 615/550/465 nm. Round-tripping them back through the shipped inversion recovers the source n,k to 0.0048, inside its 3-significant-figure precision (±0.005), so the 3-decimal rounding is the whole error budget and it sits under the measurement's own quantisation. Representative wavelengths rather than a spectral integration — this renderer is RGB with no spectral upsampling; across plausible wavelength triples `r` moves under 0.007 and `edgeTint` under 0.025 |
