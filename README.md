...
The module ships as `engine/scene/cie.h`: the CIE 1931 2° observer (CIE 018:2019) and D65 (ISO/CIE 11664-2:2022) at 1 nm over 360-830 nm, and the XYZ-to-linear-Rec.709 matrix derived from the BT.709 primaries and the tabulated D65 white (SMPTE RP 177), checked by `colour_validate`. Its first consumer, the measured-metal fit, is closed (CHANGELOG). The module is compiled into `metal_fit` and `colour_validate` only; the first runtime consumer below links it into `engine`. Remaining:

- **Out-of-gamut measured metals**: `metal_fit` rejects gold (Johnson & Christy 1974). Its CIE-projected normal-incidence red is 1.038 in linear Rec.709, because gold's yellow lies outside the BT.709 gamut, so no Gulbrandsen `r < 1` represents it and clamping would silently change both its hue and its edge tint. Copper, silver, nickel, iron and titanium all fit (worst angular residual 9.6e-3, copper red). Authoring gold needs a decision, not a tweak: either a stated gamut mapping in `metal_fit`, or a wider rendering space (e.g. ACEScg) with the OCIO scene space changed to match.
...
- **`transmissive_sphere_energy` asserts its invariant at normal incidence only**: the check renders through `renderCentre` (`tools/integrator_validate.cpp:304`), which reduces the image with `centreMean`'s centre 4x4 block (`:267`), so a sphere is scored at `mu` 1 and nowhere else. Curvature is the reason the check exists, and reading the centre samples the one point on a sphere where it costs nothing.
  - Widening the probe camera to bring the silhouette into frame (`kFocalLengthMm` 200 -> 40, `kImageSize` 16 -> 192) and binning the disc radially: at roughness 0.2 it reads **0.9450 at 90% of the silhouette radius and 0.9020 at 95%**, while its centre reads 0.9989 and passes the 3% tolerance. Roughness 0.4 reads 0.9874 / 0.9813. Roughness 0.02 holds 1.0000 through 90% and 0.9935 at 95%; 0.7 holds 0.9985 / 0.9971.
  - Two items, instrument first: a radial band assertion over the whole disc, then the ~10% grazing loss it exposes. Non-monotonic in roughness -- worst at 0.2, absent by 0.7 -- so a single grazing point sample resolves nothing and the sweep is the test.
...
- **Display-texture upload: the conversion has moved, the trace-thread cost is unresolved** (`results/wave8`): `Texture::upload` sent `GL_FLOAT` into a `GL_RGBA16F` texture, so the driver converted float-to-half serially on the render thread -- 46% of `upload_ms` on cornell, 68% on the stump. The engine now produces the half texels itself and uploads `GL_HALF_FLOAT`, which also halves the bytes transferred (18.9 MB rather than 37.7 MB at 2048x1152): `upload_ms` **0.549** on the stump, 5.59 -> 3.06 ms, of which roughly 2.1 ms is now the conversion and 1.0 ms the transfer. What is left:
  - **`pass_ms` read 1.031 in the same comparison and has to be resolved before this item can be closed.** Only 2 of 8 rounds were run, so no interval exists, but both rounds agreed. If it holds it is a net loss: it trades ~2.5 ms of render-thread time, which under vsync is slack, for ~10 ms per pass on the trace threads, which are the bottleneck. The suspected cause is the staging round trip the driver's in-place conversion did not have (~19 MB per pass of extra traffic). The fix, if needed, is to convert on the driver thread during accumulation, which requires the driver to know the displayed AOV.
  - **Tile-dirty upload is structurally impossible**, argued out rather than measured: a published pass is a running mean over every accumulated pass, so every texel of every tile changes. There is no clean tile to skip.
  - **A PBO is the one option left**, and only for the ~1.0 ms transfer half, which it defers rather than removes.
  - Recovered render-thread time is slack, not frame rate: `frame_ms` is unchanged and `swap_ms` absorbs it (§2).
...
- **Memory efficiency pass**: only monitoring exists today (Memory HUD, §2), no active reduction initiative. Scene-texture storage is now done on both axes -- data type (`textureBitDepth`) and channel count (`ImageTexture<N>`, `results/wave8`). What remains: the framebuffers, where every `HdrImage` AOV is still float RGBA whatever it encodes, and `gltf_loader.cpp`'s de-indexed mesh soup, which stores every position twice (once in `Triangle` for Embree, once in `ShadingVertex` for shading).
...
### BSDF model (not yet sequenced by wave)

- **Smith-exact transmissive multiple scattering** (distribution only, no energy error): the transmitted compensation is a Kulla-Conty shape -- the escape deficit spread over `(1 - Escape(mu_i))*cos` of its row (`kMsTransmitDensity`, `bsdf.cpp`) -- which is right in total and only approximately right in direction. Measured against a Heitz et al. 2016 stochastic Smith-microsurface random walk (GGX slopes, uniform heights), validated by its first-order reflection matching the shipped model to four digits, as total-variation distance between exit distributions over roughness 0.3/0.6/1.0 and `mu` 1/0.7/0.3:
  - `eta` 1.5: Kulla-Conty 0.052 against 0.061 for single scatter alone and 0.081 for Turquin 2019 albedo scaling -- the shipped model is the best of the three where shipped materials live (`glass.json` is 1.5168).
  - `eta` 1.33: 0.057 for both, the crossover. Below it the compensation's shape costs more than the energy it returns: at `eta` 1.01, Kulla-Conty 0.076 against 0.040 for single scatter and 0.021 for Turquin. The reference's multiply-scattered share is 13% of the exit energy on average there and 72% at roughness 1, `mu` 0.3, and it collapses toward `-wo`, which a `(1-Escape)cos` row cannot express at any scaling.
  - So this supersedes the `eta = 1` discontinuity item: `transmissionIsRough` routes an index-matched interface to the delta branch (PBRT-v4's own `eta == 1 || EffectivelySmooth()`) while the tabulated deficit either side does not tend to zero (0.20865 at `eta` 0.999, 0.21014 at 1.001, roughness 1). The total is 1 on both sides, so no furnace row can see it; what is on the far side of that jump is a shape the sweep above shows is wrong anyway below `eta` ~1.2. Switching models by `eta` would trade one arbitrary shape for another; the fix is a reference-accurate lobe.
  - Also here: the transmission `G2` is the reflection-form height-correlated term, where Heitz 2014's transmissive case is `B(1 + Lambda_o, 1 + Lambda_i)` (a Beta function). The shipped form overstates first-order transmission by up to 80%. Adopting it alone makes the total distribution worse, not better -- mean TV 0.066 to 0.098 over the same sweep -- because the energy it correctly removes from single scattering moves into the approximate compensation shape. It lands with that shape, not before it.
...
- **`-bench` captures the interactive accumulation on fast scenes**: stage 0 assumes the 0.1-scale throwaway accumulation is still running when the settle promotion fires (`kInteractiveSettleSeconds` 0.25 s, `main.cpp`). The stump (a local 20.5k-triangle test asset with 4K textures, not in the repo) reaches `maxSamples` 16 at the interactive scale in ~0.15 s, and `-bench` logs that 205x115 accumulation as the workload (`results/wave7/tree/failed_maxsamples16`). Stage 0 should complete only on a settled full-scale generation.
- **`-bench` refuses fast passes under vsync**: one `PassRecord` slot is read per frame, so two passes finishing inside one 16.7 ms frame fail the contiguity check and nothing is logged. The same stump runs at ~9 ms per interactive pass failed all 5 capped runs and passed both uncapped ones before the campaign was stopped, a selection bias toward uncapped rather than a timing result. The driver should append records to a queue the render thread drains.
...
5. **Spectral upgrade**: per-wavelength transport and hero-wavelength sampling (Wilkie et al. 2014). Dispersion already ships in RGB form (`MaterialConfig::abbe` → `bsdf.cpp`'s `cauchyIor`, one hero *channel* per path); what remains is the genuinely spectral part.
   - (a) `kRgbWavelengthsNm` (`bsdf.h:108`) is one fixed wavelength per channel -- OpenPBR's own reference implementation notes this produces "discrete RGB bands" rather than continuous rainbows, and its documented remedy is a stochastically drawn lambda per path. Not Quick: the draw needs spectral data this repo does not have.
     ...
     - Today's model is `S_c = delta(lambda - lambda_c)`, which is exactly the thing being replaced. Beyond it the only spectral data is Wave 2's CIE module (`cie.h`): the CIE 1931 observer and D65 at 1 nm and the derived XYZ-to-Rec.709 matrix, plus `kRgbWavelengthsNm` and the three Fraunhofer lines (`bsdf.cpp:446-448`).
     - The CIE tables and primary set now exist (`cie.h`, §5 Wave 2). What remains is resolving that the Rec.709 `r` sensitivity `xyzToRec709()` implies has a negative lobe -- unavoidable, since real primaries do not span the spectral locus, and not a density that can be sampled from.
...
- Walter, B. et al. (2007). Microfacet models for refraction through rough surfaces: the GGX distribution, and the rough-refraction BTDF (value eq. 21, half-vector eq. 16, Jacobian eq. 17) the transmission lobe implements in PBRT-v3's radiance-transport form. Sec. 5.3's per-microfacet reflect/refract choice, weighted by that facet's Fresnel, is what `facetReflectProbability` (`bsdf.cpp`) implements, so both branches weigh G2/G1 rather than the macro split's F(wo.h)/R_ss (PBRT-v4's `DielectricBxDF` makes the same choice).
...
- Johnson, P.B., Christy, R.W. (1974). Optical constants of transition metals: Ti, V, Cr, Mn, Fe, Co, Ni, and Pd. Phys. Rev. B 9, 5056: the measured chromium `(n, k)` behind `chrome.json` (`tools/data/chromium_johnson_christy_1974.csv`, via the refractiveindex.info database).
- CIE 018:2019, The Basis of Physical Photometry, Table 6 (ISO/CIE 11664-1:2019): the CIE 1931 2° colour-matching functions at 1 nm, `src/scene/cie_1931.inc`.
- ISO/CIE 11664-2:2022, Colorimetry -- Part 2: CIE Standard Illuminants, Table B.1: D65 at 1 nm, `src/scene/cie_1931.inc`.
- CIE 015:2018, Colorimetry, 4th ed.: tristimulus integration of a reflectance under an illuminant at the 1 nm summation interval, `cie::reflectanceToRec709`.
- ITU-R BT.709-6 (2015): the Rec.709 primary chromaticities; SMPTE RP 177-1993: deriving the RGB-to-XYZ matrix from primaries and white, `cie::xyzToRec709`.
- Gulbrandsen, O. (2014). Artist Friendly Metallic Fresnel. JCGT 3(4): the conductor Fresnel parameterisation, ported from the paper's Appendix A. Reflectivity `r` (the existing `f0`) plus an authored `edgeTint` `g` invert to a complex IOR via eq 12 and eq 2, replacing Schlick, which is monotone in `cos` by construction and so forces every metal to exactly white at grazing. Two documented departures from the listing: `k^2` is evaluated in the factored form `(nMax-n)(n-nLow)` (the same expression through eq 2's own roots) because the literal form loses all precision in float32 near `r=1`, returning `-1.28e6` where the truth is 0; and the reflectance itself is the exact unpolarised complex-IOR Fresnel rather than the listing's large-|eta| approximation, which deviates from it by up to 0.094 absolute at mid reflectivity. The cosine mean `F_avg` the Kulla-Conty tint needs has no closed form on this basis and is a 3-node quadrature rule over the same Fresnel, fitted against 128-point Gauss-Legendre to a measured 4.0e-4 (`conductorFresnelAvg`, asserted by `checkAverageFresnel`); a rational fit over `(r, g)` was measured and rejected, since `n` collapses from 39.8 to 0.005 across the last tenth of `g` at `r=0.99` and 49 terms reached only 9e-3. The same rule, unrefitted, serves the dielectric interface over `fresnelDielectric` (`dielectricFresnelAvg`): measured worst 5.5e-5 over `ior` in [1.05, 3.0] against 6.5e-3 for the two-constant rational fit it replaced, and exactly `+0` at `ior` 1 because every node then evaluates an index-matched interface. The exact closed form of the dielectric integral (d'Eon & Irving 2011) was rejected -- it would re-derive the interface in a second place, its accuracy is unobservable against a table-limited instrument, and its logarithmic terms cancel catastrophically in the same `ior -> 1` band the rule is weakest in.
...
- Heitz, E., Hanika, J., d'Eon, E., Dachsbacher, C. (2016). Multiple-scattering microfacet BSDFs with the Smith model. Also the reference instrument: its stochastic random walk on the Smith microsurface is ground truth for the exit distribution a rough dielectric actually has, which is how §5's Smith-exact entry measures the shipped compensation's shape (and how Turquin's scaling and the Beta-form transmission G2 were evaluated and declined). The source of the problem, not the solution used: it establishes the energy single-scatter GGX discards (a white conductor returned 0.31 of the light it received at roughness 1.0), but the implemented compensation is Kulla & Conty's cheaper directional-albedo form below rather than this paper's stochastic microsurface evaluation.
- Turquin, E. (2019). Practical multiple scattering compensation for microfacet models. Evaluated against the Heitz 2016 reference and not adopted: scaling the single-scatter lobe by 1/E_ss keeps its shape, which beats the Kulla-Conty row near index match (mean TV 0.021 against 0.076 at `eta` 1.01) and loses at the indices materials actually use (0.081 against 0.052 at 1.5). Numbers and the crossover in §5.
