...
- **`-bench` frame pacing is not vsync-bound**: one default-profile convergence measured a median `frame_ms` of 10.9 ms (p10 3.2 ms) against a reported 60 Hz display, so per-frame columns (`frame_ms`, `swap_ms`, the HUD stages) and their totals depend on something outside the config. Per-event columns (`upload_ms`, one entry per upload; `pass_*`, one per pass) are unaffected. Cause not measured; window occlusion is one candidate. Establish it before comparing any per-frame total across builds -- Wave 5's display-texture-upload item relies on the per-event columns, which are unaffected, but any future per-frame comparison needs this settled first.
- **`driver.running_mean_matches_batch_mean` is nondeterministic at two threads**: measured 10 failures in 50 runs on `main` and 11 in 50 on the branch that found it, at one fixed seed and `--threads=2`, so it is the check or the driver, not a seed. It is labelled `exact`, which a check that fails one run in five is not. Find whether the running mean's per-pass accumulation order varies with pool scheduling (which would make the Higham bound the wrong instrument) or the bound is simply too tight, before trusting any red from it.
...
- **`msTransmit`'s selection gate and its value gate test different quantities** (bias): `computeLobeProbabilities` (`bsdf.cpp`) allocates selection mass when `1 - escapeAvg > kMinDeficit`, at the forward orientation `eta = etaI/etaT`, while the value being sampled switches off in `multiScatterShape` when `1 - escapeAvgRecip <= kMinDeficit`, at the reciprocal one. The comment at the gate claims these are the same test. They are not: escape is not symmetric in eta, measured 0.00036 at eta 0.941 against 0.00105 at its reciprocal 1.063 at roughness 0.129, straddling `kMinDeficit`.
  - Where the value gate opens and the selection gate does not, the transmitted share is delivered over the whole far hemisphere with nothing sampling it -- the exact condition `multiScatterShape`'s own doc comment says would "bias the estimator rather than merely darken it". The reverse case only spends selection mass on a lobe of identically zero value.
  - Not caught by any validator today, including the new `checkSamplingChiSquare`, which tests the density a strategy draws from and not whether the strategy exists. The exact sampler did not address it: that changed the shape drawn, not which gate decides whether to draw at all. Needs the reachable band measured before choosing to align both gates on the reciprocal average or on the pair.
- **The transmit-side escape table is deliberately under-converged** (white slab 2.4% over): `tools/albedo_table.cpp` bakes `r`/`t`/`ravg`/`tavg` at 16x16 stratified samples per cell, the count they had when cost still mattered, even though offline they are free.
  - Raising it to 128x128 drops the escape total at roughness 1 by 0.00165 (0.81216 vs 0.81381 at eta index 5), the compensation returns that much more, and `integrator_validate`'s white slab goes 1.02428 -> 1.03017, past its 0.03 band. The slab was already 2.4% over before any of this, so the coarse table was partly funding that error rather than avoiding it.
  - That localises the rough-transmission over-energy to the transmit compensation itself rather than to MIS double-counting, which is what the failure message assumes. Reproduce with `./build/albedo_table --samples 128`; the fix belongs with the rough-transmission items below, and the count goes up once it lands.
- **The exiting side's reflected multiple-scattering share is VNDF-sampled** (variance only): `diffuseProb` is 0 inside a transmissive medium, so `msReflect` is 0 there too and the internally-reflected compensation lobe is drawn by the specular strategy alone (`bsdf.cpp`). Coverage is complete for `alpha > 0`, so this is variance and not bias, and the far-hemisphere twin `msTransmit` is unaffected. Needs a measurement inside rough glass before a third reflection strategy is worth its selection mass.
...
### Wave 4: Relax `aovNeedsLightTransport` gating (one enabler, four consumers)

Unblocks the delighted DirectDiffuse view, the path-traced Fresnel AOV, the contact sheet, and AOV-switch restarts (the last one measured via the benchmark log, §2).
...
- **Depth of field**: thin-lens sampling in `primaryRay` + focus distance; technically unblocked today, cheaper once adaptive sampling lands (Wave 5).
...
- **Reduce the HUD's build cost**: the `hud build` / `hud render` split (`-stats`) now attributes the stage, and the cost is **entirely widget construction in `HudOverlay::draw`**, not draw-list submission: 1.520 ms against 0.107 ms with the HUD shown, 0.000 ms against 0.021 ms with it hidden (cornell, 2048x1152, Apple M1). Any optimisation belongs in `draw`; `ImGui::Render`/`RenderDrawData` has nothing worth attacking.
  ...
  - Deprioritised rather than open: the recovered 1.5 ms goes to `swap (vsync)` (48.6% -> 68.8% of a `cpu total` that barely moves, 8.576 -> 8.463 ms), not to frame rate. At 60 Hz the render thread is vsync-bound with ~8 ms of headroom, so this is slack being spent, not budget. It becomes worth doing when something else makes the render thread CPU-bound -- most likely the Depth auto-range scan below, which is 5.8x larger.
...
- Kalibera, T., Jones, R. (2013). Rigorous Benchmarking in Reasonable Time. ISMM: the process invocation, not the iteration, as the unit of replication -- why each log record keeps raw samples but `bench_compare` compares per-invocation totals.
