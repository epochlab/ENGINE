...
1. **Volumetric & subsurface transport** — Participating media (fog/smoke/atmosphere: extinction/scattering coefficients, phase functions, ray-marched or null-collision free-flight sampling) and subsurface scattering (translucent materials: a BSSRDF or random-walk diffusion approximation) are both additional transport modes layered on the existing surface path tracer — extending the same NEE/MIS machinery to volume-embedded and beneath-the-surface light paths rather than replacing it. One BSDF limitation blocks the bidirectional half of this: the transmit-side multiple-scattering lobe is not reciprocal, since it evaluates the `wi`-side escape albedo at the `wo`-side `eta` — which is exactly what makes its energy come out right — so `f(wo->wi)` and `f(wi->wo)` disagree through rough glass. `tools/bsdf_validate.cpp`'s `checkTransmissionReciprocity` is scoped to single scatter for that reason, and the lobe needs reworking before BDPT/VCM/photon mapping can carry a subsurface or volumetric path through a transmissive interface.
2. **Reference scenes & material validation** — A Cornell box (Goral et al. 1984) authored as a scene the renderer is *measured against* rather than merely rendered in: known geometry, an emissive ceiling panel, and a published reference to compare colour bleeding and energy conservation against. Today `tools/` validates the BSDF and integrator analytically (furnace tests, a single unoccluded quad, a two-quad slab) and nothing validates a full scene, so a whole class of transport error — bucket misattribution, light leaks at shared edges, terminator artifacts on curved geometry — is only visible by eye. Hard dependency on area lights (item 0): the environment map is currently the only light source, so an emissive panel cannot be authored at all. Carries the per-material showcase alongside it — mirror and rough conductor, smooth and rough dielectric refraction (both implemented), and a subsurface material once item 1 lands.
...
8. **Production-scale scene/asset pipeline** — Out-of-core geometry streaming and a real multi-object/instanced scene graph (today: one configured glTF asset and one HDRI, loaded whole into memory at startup, with no scene graph beyond that single file's own node hierarchy) needed before scenes larger than a single asset are practical; deterministic distributed rendering across machines for feature-length throughput. Broader material coverage — layered BSDFs, hair/fur, cloth — belongs here too, alongside whatever transport (items 0–1 above) those material types need to look correct.
9. **Lens and shutter sampling — depth of field, motion blur** — Both are Cook/Porter/Carpenter 1984 distributed ray tracing: extra stochastic dimensions on the *primary ray*, integrated per sample and converging with the rest of the image, not a post-process filter over Beauty the way item 5's chromatic aberration is. `Camera` already carries `aperture()`, `focalLengthMm()` and `shutterSeconds()`, but they feed the EV100 exposure model only (§2) — f/1.4 currently brightens the image without shallowing depth of field, and the shutter speed never reaches the sampler. Depth of field is the smaller half and is technically unblocked today: thin-lens sampling in `primaryRay` plus an authored focus distance. It sits here rather than earlier because it adds variance to every pixel, so it wants item 4's adaptive per-pixel sample budget ahead of it to pay for itself. Motion blur is the larger half and genuinely is blocked: it needs time-varying transforms, which means the scene graph from item 8, plus Embree's multi-timestep geometry to trace against.
...
- Goral, C.M., Torrance, K.E., Greenberg, D.P., Battaile, B. (1984). Modeling the interaction of light between diffuse surfaces. SIGGRAPH — the Cornell box, named in §4's reference-scenes roadmap item, not yet implemented (no area lights exist to author its emissive panel with).
...
- Cook, R.L., Porter, T., Carpenter, L. (1984). Distributed Ray Tracing. SIGGRAPH — the stochastic-sampling origin of depth of field and motion blur, named in §4's lens-and-shutter roadmap item, not yet implemented (primary rays are pinhole and instantaneous; the camera's aperture and shutter drive exposure only).
