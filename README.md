...
- **Print the hotkey map to the terminal**: hotkeys exist only as an edge-triggered GLFW callback (`main.cpp:542-568`: L/R/G/B/0/I/H/ESC), documented solely in a code comment, with no in-app surface. Small addition alongside the item above: print the hotkey table to stdout, on a keypress (e.g. `?`/`H`) or at startup.
