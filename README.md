...
- **The transmit-side escape table is deliberately under-converged**: `tools/albedo_table.cpp` bakes `r`/`t`/`ravg`/`tavg` at 16x16 stratified samples per cell, the count they had when cost still mattered, even though offline they are free.
...
- **`dielectricFresnelAvg`'s rational fit is now the limit on the coat**: `checkCoatFresnelAvg` (`tools/bsdf_validate.cpp`) recovers the `F_avg` the coat actually used -- by dividing the diffuse channel by `referenceEon` and inverting the coupling -- and lands 6.6e-5 from `dielectricFresnelAvg`'s own value. What the check then spends its tolerance on is not the instrument but the fit: 0.00228 at ior 1.5 falling to 0.00149 at 1.55, against Karis' 0.00606 and 0.00714.
